...
const dataPointHandler = (data) => {
  ...
  // 按照dom分组，且记录每个点的点击次数
  data.forEach((item) => {
    const { dom, clickxpos, clickypos } = item
    const pointIndex = `${clickxpos}_${clickypos}`
    ...
    if (res[dom]) {
      ...
      if (res[dom][pointIndex]) {
        ...
        res[dom][pointIndex] = { clickxpos, clickypos, max: 1 }
      }
    } else {
      // 若不存在dom，则创建分组对象
      res[dom] = { [pointIndex]: { clickxpos, clickypos, max: 1 } }
...
const createOwnRoutes = (router) => {
  router.post('/heatmapClickPoint', async (ctx, next) => {
    ...
    const sql = `select dom, clickxpos, clickypos from (${unions}) as value where route = '${route}'`
